<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
                <h2 class="text-2xl font-bold text-slate-900">Rekap Bulanan {{ $kelas->nama_kelas }}</h2>
                <p class="mt-1 text-sm text-slate-400">Tahun Ajaran {{ $kelas->tahunAjaran->tahun_ajaran ?? '-' }}</p>
            </div>
            <div class="flex flex-wrap gap-2">
                <a href="{{ route('wali.laporan.show', $kelas->id) }}"  class="inline-flex items-center rounded-full border border-slate-700 px-4 py-2 text-sm font-medium text-slate-200 hover:bg-slate-800/70 transition">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                    </svg>
                    Rekap Harian
                </a>
                <a href="{{ route('wali.laporan.bulanan-export', ['kelas' => $kelas->id, 'bulan' => $bulanDipilih]) }}" class="inline-flex items-center rounded-full border border-sky-600 bg-sky-600/10 px-4 py-2 text-sm font-semibold text-sky-200 hover:bg-sky-600/20 transition">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="py-8">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="rounded-2xl border border-slate-800 bg-slate-900/80 p-6 shadow-2xl shadow-black/40">
                <form method="GET" action="{{ route('wali.laporan.bulanan', $kelas->id) }}" class="grid gap-4 md:grid-cols-3">
                    <div>
                    ...
                    <div class="md:col-span-2 flex items-end gap-3">
                        ...
                        <a href="{{ route('wali.laporan.bulanan-export', ['kelas' => $kelas->id, 'bulan' => $bulanDipilih]) }}"class="inline-flex items-center rounded-xl border border-slate-700 px-5 py-2.5 text-sm font-semibold text-slate-200 hover:bg-slate-800/70 transition">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v12m0 0l-4-4m4 4l4-4m-8 8h8" />
                            </svg>
                            PDF Bulan Ini
