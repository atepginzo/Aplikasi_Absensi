            ...
            <form method="GET" action="{{ route('admin.laporan.index') }}" class="mb-6">
                <div class="flex flex-col sm:flex-row sm:items-end gap-4 bg-slate-900/80 backdrop-blur-sm border border-slate-800 rounded-2xl p-4">
                    ...
                    @if ($tahunAktif && request('tahun_ajaran_id') && request('tahun_ajaran_id') != $tahunAktif->id)
                        <a href="{{ route('admin.laporan.index') }}" class="inline-flex items-center px-5 py-2.5 text-sm font-semibold text-slate-300 rounded-xl border border-slate-700 hover:bg-slate-800 transition">
                            Reset
                        </a>
                    @endif
                </div>
