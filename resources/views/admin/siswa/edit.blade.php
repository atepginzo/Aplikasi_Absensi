    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-400">Ubah informasi data siswa</p>
            </div>
        </div>
    </x-slot>
                    ...
                    <form method="POST" action="{{ route('admin.siswa.update', $siswa->id) }}" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="col-span-1 md:col-span-2">
                                ...
                                <select id="kelas_id" name="kelas_id" class="block w-full rounded-xl bg-slate-800/50 border border-slate-700 text-slate-200 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-900 transition-all duration-200 text-sm px-4 py-2.5" required>
                                    ...
                                    @foreach ($semuaKelas as $kelas)
                                        <option value="{{ $kelas->id }}" @selected(old('kelas_id', $siswa->kelas_id) == $kelas->id) class="bg-slate-800">
                            ...
                            <div>
                                ...
                                <select id="jenis_kelamin" name="jenis_kelamin" class="block w-full rounded-xl bg-slate-800/50 border border-slate-700 text-slate-200 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-900 transition-all duration-200 text-sm px-4 py-2.5" required>
                                    <option value="L" @selected(old('jenis_kelamin', $siswa->jenis_kelamin) == 'L') class="bg-slate-800">Laki-laki</option>
                                    <option value="P" @selected(old('jenis_kelamin', $siswa->jenis_kelamin) == 'P') class="bg-slate-800">Perempuan</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="mt-6 p-4 bg-slate-800/50 border border-slate-700 rounded-xl">
                            <div class="flex items-start">
                                ...
                                <div>
                                    <p class="text-sm font-semibold text-slate-200 mb-1">QR Code Token</p>
                                    <p class="text-xs text-slate-300 font-mono bg-slate-900/50 px-3 py-2 rounded-lg border border-slate-700 inline-block">
                                        {{ $siswa->qrcode_token }}
                                    </p>
                                    <p class="text-xs text-slate-400 mt-2">Token ini tidak dapat diedit manual</p>
                                </div>
                            </div>
                        </div>
